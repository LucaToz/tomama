...
import { SONGS, VAULT_CODE, normalizeSong } from "./songs.js";
...
async function readConfigFromBlob() {
  try {
    ...
    // Il blob pubblico è servito via CDN: bustiamo la cache con uploadedAt
    // così ogni salvataggio admin produce una URL diversa e dati freschi.
...
async function writeConfigToBlob(config) {
  ...
  await put(BLOB_CONFIG_PATH, body, {
    ...
    // Minimo consentito da Vercel Blob (0 = default 1 anno di cache CDN).
